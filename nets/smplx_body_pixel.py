...
class TrainWrapper(TrainWrapperBaseClass):
    ...
    def __init__(self, args, config):
        ...
        self.pose_style_encoder = StyleEncoderbody2().to(self.device)
        if self.audio:
    ...
    def __call__(self, bat):
        ...
        # print('____',poses.shape) ##torch.Size([128, 165, 88])
        # style_code = self.pose_style_encoder(poses)
        # print('....',styles.shape) ##torch.Size([128, 4])
        # style_code = style_code.to(self.device).to(torch.int64)
        # print('......+++',styles)###torch.Size([128])
        # id = bat['speaker'].to(self.device) - 20
        # print(id.dtype)
        # print('......+++',id)
        # print('......++++++++',id.shape)#torch.Size([128])
        # id = F.one_hot(id, self.num_classes)
        poses = poses[:, self.c_index, :]

        aud = aud.permute(0, 2, 1)
        gt_poses = poses.permute(0, 2, 1)
        # print('\\',gt_poses.shape)#([128, 88, 129])
        style_code = self.pose_style_encoder(gt_poses)
        # print('....',style_code.shape) ##([128, 512])
        style_code = style_code.sum(dim=1).to(self.device).to(torch.int64)

        with torch.no_grad():
            self.g_body.eval()
            self.g_hand.eval()
            if torch.cuda.device_count() > 1:
                _, body_latents = self.g_body.module.encode(gt_poses=gt_poses[..., :self.each_dim[1]], style_code=style_code)
                _, hand_latents = self.g_hand.module.encode(gt_poses=gt_poses[..., self.each_dim[1]:], style_code=style_code)
            else:
                _, body_latents = self.g_body.encode(gt_poses=gt_poses[..., :self.each_dim[1]], style_code=style_code)
                _, hand_latents = self.g_hand.encode(gt_poses=gt_poses[..., self.each_dim[1]:], style_code=style_code)
        ...
        if self.audio:
            ...
            logits = self.generator(latents[:, :], style_code, audio)
        else:
            logits = self.generator(latents, style_code)
    ...
    def infer_on_audio(self, aud_fn, initial_pose=None, norm_stats=None, exp=None, var=None, w_pre=False, rand=None,
                       continuity=False, style_code=None, fps=15, sr=22000, B=1, am=None, am_sr=None, frame=0,**kwargs):
        '''
        ...
        # styles = torch.tensor(styles, dtype=torch.float32, device=self.generator.device)
        if style_code is None:
            style_code = torch.tensor([0]).to(self.device)
        else:
            style_code = style_code.repeat(B)
        with torch.no_grad():
            ...
            if continuity:
                ...
                pre_latents, pre_audio, body_0, hand_0 = self.infer(aud_feat[:, :gap], frame, style_code, B, pre_pose=pre_pose)
                pre_pose['b'] = body_0[:, :, -4:].transpose(1,2)
                pre_pose['h'] = hand_0[:, :, -4:].transpose(1,2)
                _, _, body_1, hand_1 = self.infer(aud_feat[:, gap:], frame, style_code, B, pre_latents, pre_audio, pre_pose)
                ...
                if self.audio:
                    ...
                    latents = self.generator.generate(style_code, shape=[audio.shape[2], 2], batch_size=B, aud_feat=audio)
                else:
                    latents = self.generator.generate(style_code, shape=[aud_feat.shape[1]//4, 2], batch_size=B)
    ...
    def infer(self, aud_feat, frame, style_code, B, pre_latents=None, pre_audio=None, pre_pose=None):
        audio = self.audioencoder(aud_feat.transpose(1, 2), frame_num=frame).unsqueeze(dim=-1).repeat(1, 1, 1, 2)
        latents = self.generator.generate(style_code, shape=[audio.shape[2], 2], batch_size=B, aud_feat=audio,
    ...
    def generate(self, aud, style_code, frame_num=0):
        ...
        if self.audio:
            ...
            latents = self.generator.generate(style_code, shape=[audio.shape[2], 2], batch_size=aud.shape[0], aud_feat=audio)
        else:
            latents = self.generator.generate(style_code, shape=[aud_feat.shape[1] // 4, 2], batch_size=aud.shape[0])
